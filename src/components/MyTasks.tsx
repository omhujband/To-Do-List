...
export const MyTasks: React.FC = () => {
    ...
    return (
        <div className="min-h-full bg-base flex flex-col text-text-main">
            <header className="px-8 py-8 shrink-0 border-b border-border-main bg-surface/50">
                ...
                <form onSubmit={handleAddTask} className="flex gap-3 max-w-2xl">
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            className="w-full bg-surface-hover border border-border-main rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-primary text-text-main placeholder-text-muted transition-colors"
                        />
                    </div>
                    <button
                        type="submit"
                        disabled={!newTaskTitle.trim()}
                        className="bg-primary hover:bg-primary-hover disabled:opacity-50 text-white px-6 py-3 rounded-xl font-semibold transition-colors"
                    >
                        Add Task
                    </button>
                </form>
            ...
            <main className="flex-1 overflow-y-auto px-8 py-8 custom-scrollbar">
                {(state.globalTasks || []).length === 0 && allTasks.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-64 text-text-muted">
                        ...
                        <h3 className="text-xl font-semibold text-text-muted">No tasks found</h3>
                    ...
                    <div className="space-y-8 pb-12">
                        ...
                        {(state.globalTasks || []).length > 0 && (
                            <div className="bg-surface border border-primary/20 rounded-3xl p-6">
                                <h2 className="text-xl font-bold text-text-main mb-6 flex items-center gap-3">
                                    <div className="w-2 h-6 bg-primary rounded-full" />
                                    Today's List
                                    <span className="text-xs font-semibold px-2 py-1 bg-primary-light text-primary rounded-md ml-auto border border-primary/20">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                            {(state.globalTasks || []).map(task => (
                                                <div key={task.id} className="flex flex-col gap-2">
                                                    <div className="text-xs font-semibold text-primary px-1 uppercase tracking-wider">
                                                        Independent Task
                                                    </div>
                                                    <TaskCard
                                                        card={task}
                        ...
                        {state.workspaces.map(workspace => {
                            ...
                            return (
                                <div key={workspace.id} className="bg-surface border border-border-main rounded-3xl p-6">
                                    <h2 className="text-xl font-bold text-text-main mb-6 flex items-center gap-3">
                                        <div className="w-2 h-6 bg-border-main rounded-full" />
                                        {workspace.title}
                                        <span className="text-xs font-semibold px-2 py-1 bg-surface-hover text-text-muted rounded-md ml-auto">
                                        ...
                                        <SortableContext items={workspaceTasks.map(t => t.id)} strategy={horizontalListSortingStrategy}>
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                                {workspaceTasks.map(task => (
                                                    <div key={task.id} className="flex flex-col gap-2">
                                                        <div className="text-xs font-semibold text-text-muted px-1 uppercase tracking-wider">
